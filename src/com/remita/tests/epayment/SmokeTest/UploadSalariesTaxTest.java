...
import Util.TestUtility;
	...
	public class UploadSalariesTaxTest extends TestBase {
		...
		// 2nd Step to parameterization
			...
			// 3rd Step
			public UploadSalariesTaxTest(String pensionMonth,  String pensionYear, String briefDesc, String detailedDesc, String FundAcct, String positiveData ){
		...
		@Test
		public void uploadSalariesTaxTest() throws IOException, InterruptedException{
			...
			ApplicationLogs.debug("Upload Salaries Tax Module: Moved mouse to the Main Menu Link");
			...
			ApplicationLogs.debug("Upload Salaries Tax Module: Clicked the Pay Pension Menu");
			...
			ApplicationLogs.debug("Upload Salaries Tax Module: Switch into the Content iFrame");
			
					
			driver.findElements(By.xpath("//input[@name='salOption']")).get(2).click();
			
			Thread.sleep(5000);
			
			new Select(getObjectById("drd_PensionMonth")).selectByValue(pensionMonth);
			ApplicationLogs.debug("Upload Salaries Tax Module: Selected Pension Month");
			
			new Select(getObjectById("drd_PensionYear")).selectByValue(pensionYear);
			ApplicationLogs.debug("Upload Salaries Tax Module: Selected Pension Year");
			
			getObjectByName("btn_Browser").click();
			ApplicationLogs.debug("Upload Salaries Tax Module: Clicked Browse Button");
			
			Process process = new ProcessBuilder(System.getProperty("user.dir")+"\\src\\config\\file_upload6.exe",
								"C:\\Users\\USER\\Documents\\uploadfolder\\taxOnly (1).csv", "Open").start();
			ApplicationLogs.debug("Upload Salaries Tax Module: Selected and Attached the Pension CSV File");
			...
			ApplicationLogs.debug("Upload Salaries Tax Module: Your File has been successfully uploaded.");
			ApplicationLogs.debug(actualText);
			
			getObjectByXpath("lnk_ProcessTransaction").click();
			ApplicationLogs.debug("Upload Salaries Tax Module: Clicked to process transactions");
			
			new Select(getObjectByName("drd_FundingBank")).selectByValue(FundAcct);
			
			
			ApplicationLogs.debug("Upload Salaries Tax Module: Selected Funding banks and Pension Account");
			
			getObjectByName("btn_SubmitPension").click();
			ApplicationLogs.debug("Upload Salaries Tax Module: Clicked Submit Payment to Pension Button");
